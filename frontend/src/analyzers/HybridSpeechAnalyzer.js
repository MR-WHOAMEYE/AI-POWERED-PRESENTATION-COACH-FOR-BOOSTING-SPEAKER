...
export class HybridSpeechAnalyzer {
    ...
    async start() {
        ...
        try {
            ...
            // Prefer Web Speech API by default for instant, zero-latency streaming transcription
            if (this.isWebSpeechSupported() && options.preferredMode !== 'backend') {
                this.mode = 'webspeech';
                this.startWebSpeechTranscription();
            } else {
                // Check backend availability for Whisper fallback
