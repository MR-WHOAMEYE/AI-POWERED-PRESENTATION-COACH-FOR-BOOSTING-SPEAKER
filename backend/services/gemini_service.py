...
class GeminiService:
    ...
    def __init__(self):
        ...
        enable_search = getattr(Config, 'GEMINI_ENABLE_SEARCH', False)
        ...
        try:
            if tools:
                ...
                print(f"[OK] Gemini initialized with model '{model_name}' and Google Search grounding.")
            else:
                self.model = genai.GenerativeModel(model_name)
                print(f"[OK] Gemini initialized with model '{model_name}'.")
        except Exception as e:
            print(f"[WARNING] Could not initialize model '{model_name}': {e}. Falling back to 'gemini-2.5-flash'.")
